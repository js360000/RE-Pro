...
# Late-binding indirection: tests patch ``re_pro.gui.list_profiles`` and
# ``re_pro.gui.load_profile`` on the package namespace. ``_pkg.NAME`` lookups
# happen at call time, so the patches flow through to call sites here.
from re_pro import gui as _pkg
...
from ..profiles import analysis_settings_from_profile, build_analysis_profile, save_profile
...
class MainWindow(QMainWindow):
    ...
    def _refresh_profiles(self) -> None:
        ...
        if selected_kind == "All":
            selected_kind = ""
        self._profile_entries = _pkg.list_profiles(query=query, profile_type=selected_kind.lower())
    ...
    def _show_selected_profile(self, current: QListWidgetItem | None, previous: QListWidgetItem | None) -> None:
        ...
        if not path:
            ...
            return
        try:
            profile = _pkg.load_profile(path)
